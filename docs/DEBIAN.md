...
sudo apt install ../rgbdns_0.3.2_$(dpkg --print-architecture).deb
```
...
cd rgbdns
packaging/build-deb.sh
dpkg-deb --info ../rgbdns_0.3.2_amd64.deb
```

Copy the package to the EC2 host, then install it there:

```sh
scp ../rgbdns_0.3.2_amd64.deb admin@52.10.53.234:/tmp/
ssh admin@52.10.53.234
sudo apt update
sudo apt install -y /tmp/rgbdns_0.3.2_amd64.deb
dpkg-query -W rgbdns
```
