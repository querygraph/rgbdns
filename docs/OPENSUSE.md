...
rpm -qip dist/rpmbuild/RPMS/x86_64/rgbdns-0.1.1-2.x86_64.rpm
rpm -qlp dist/rpmbuild/RPMS/x86_64/rgbdns-0.1.1-2.x86_64.rpm
```
...
```sh
rpm -K dist/cloud-rpm/rgbdns-0.1.1-2.x86_64.rpm
rpm -qip dist/cloud-rpm/rgbdns-0.1.1-2.x86_64.rpm
rpm -qlp dist/cloud-rpm/rgbdns-0.1.1-2.x86_64.rpm
```
...
```sh
scp -i ~/.ssh/KEY.pem \
  dist/rpmbuild/RPMS/x86_64/rgbdns-0.1.1-2.x86_64.rpm \
  ec2-user@PUBLIC_ADDRESS:/tmp/
ssh -i ~/.ssh/KEY.pem ec2-user@PUBLIC_ADDRESS
sudo zypper --non-interactive --no-gpg-checks install \
  /tmp/rgbdns-0.1.1-2.x86_64.rpm
rpm -q rgbdns
rpm -V rgbdns
```
