...
export async function POST(request: Request, context: { params: Promise<{ name: string }> }) {
  ...
  await sql.begin(async (transaction) => {
    if (rotate) await transaction.unsafe("UPDATE acme_credentials SET status = 'revoked', secret_ciphertext = '', secret_iv = '', revoked_at = now() WHERE domain_id = $1 AND status = 'active'", [domain.id]);
  ...
  let policyDeployment = "dispatched";
  try { await dispatchDnsDeployment(); } catch { policyDeployment = "pending"; }
  ...
  return Response.json({ keyName, ini, policyDeployment }, { status: 201, headers: { "cache-control": "no-store" } });
}

export async function DELETE(request: Request, context: { params: Promise<{ name: string }> }) {
  const principal = await authenticate(request); if (!principal) return unauthorized();
  const { name } = await context.params; const sql = db();
  const revoked = await sql`UPDATE acme_credentials AS credential SET status = 'revoked', secret_ciphertext = '', secret_iv = '', revoked_at = now() FROM domains AS domain WHERE credential.domain_id = domain.id AND domain.account_id = ${principal.accountId} AND domain.name = ${name.toLowerCase()} AND credential.status = 'active' RETURNING credential.id`;
  if (!revoked.length) return Response.json({ error: "No active credential found" }, { status: 404 });
  let policyDeployment = "dispatched";
  try { await dispatchDnsDeployment(); } catch { policyDeployment = "pending"; }
  return Response.json({ revoked: revoked.length, policyDeployment });
}
